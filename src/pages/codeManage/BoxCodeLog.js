...
import { Table, Form, Input, Spin, AutoComplete, Select, Icon, Button, Divider, Popconfirm, Dropdown, Menu, DatePicker, Modal, Checkbox } from "antd";
import { pagination } from '../../utils/pagination';
import Toast from '../../utils/toast';
import { SubmitForm } from '../../components/common-form';
import dateUtil from '../../utils/dateUtil';
import { searchBoxLogList, deleteBoxLog, exportUniqueCodeLog, stopDrawQrCode, generateQrcodeImage, batchExportUniqueCode, submitParam, checkExportUniqueCode } from '../../api/codeManage/boxCode';
...
const _exampleData = {
  traceCode: "http://h5.trace.adorsmart.com/code/product?code=X8534460072900004920990800030d42&key=2f3d84035c8dbef095c0310cf0f22f7b@",
  boxCode: "853446007291",
  produceCode: "0004920190800030d41",
  uniqueCode: "X8534460072900004920990800030d42"
}
...
class Page extends Component {
  ...
  getPageData = (isInit) => {
    ...
    searchBoxLogList(this.params).then(res => {
  ...
  columns = [
    ...
    { title: "已生成唯一码数量", dataIndex: "exportQuantity" },
      ...
      <div>
        ...
        >
          <Spin spinning={this.state.exportLoading}>
            ...
            <div style={{ wordBreak: "break-all" }}>全码：{_exampleData.traceCode}</div>
            <div className='color-red' >（箱规全码即二维码指向的链接）</div>
            <div>箱规编码：{_exampleData.boxCode}</div>
            <div className='color-red'>（箱规编码在生成箱规时设置的，是箱规唯一码的一部分）</div>
            <div>箱规生产码：{_exampleData.produceCode}</div>
            <div className='color-red'>（箱规生产码是箱子唯一码的一部分，包含企业编号、生成时间戳及序列号）</div>
            <div>箱规唯一码：{_exampleData.uniqueCode}</div>
            <div className='color-red'>（箱规唯一码是箱子的唯一性标识：箱规唯一码=X+箱规编码+箱规生产码）</div>
