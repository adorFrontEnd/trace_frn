...
import { searchProduceScanQuantityList, exportProduceScanQuantity } from '../../api/scan/produceScan';
...
const detailPath = routerConfig["o2oManage.activityCouponManage"].path;
...
class OperLog extends Component {
  ...
  getPageData = (params) => {
    ...
    searchProduceScanQuantityList(this.params).then(res => {
  ...
  columns = [
    ...
    { title: "扫码商品码", dataIndex: "productQuantity", render: data => data || data == 0 ? <span className={data == 0 ? "color-gray" : "font-bold font-16 color-red"}>{data}</span> : "--" },
    { title: "扫码箱码", dataIndex: "boxQuantity", render: data => data || data == 0 ? <span className={data == 0 ? "color-gray" : "font-bold font-16 color-red"}>{data}</span> : "--" },
    { title: "扫描托盘码", dataIndex: "trayQuantity", render: data => data || data == 0 ? <span className={data == 0 ? "color-gray" : "font-bold font-16 color-red"}>{data}</span> : "--" },
    { title: "操作", dataIndex: "status", render: (text, record) => <a onClick={() => { this.goDetail(record) }}>查看详情</a> }
  ]
  ...
  exportProduceScanQuantity = ()=>{
    ...
    exportProduceScanQuantity(this.params);
  }
  /**渲染**********************************************************************************************************************************/
    ...
    return (
      <div>
        ...
        <div className='flex-between padding10-0 align-center'>
          <div className='line-height40 color-red padding-left' style={{ width: 230 }}>当日生产扫描记录会在第二日显示</div>
          <Form layout='inline'>
            <Form.Item label='商品'>
              {
            ...
            <Form.Item>
              <Button className='yellow-btn' onClick={this.exportProduceScanQuantity}>导出</Button>
            </Form.Item>
          </Form>
        </div>
        <Table
